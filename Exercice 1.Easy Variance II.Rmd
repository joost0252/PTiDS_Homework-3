...
Bootstrap <- function(data, B, Statistic) {
  ...
  # Error and Warning message 
  if (Statistic == "var") {
    stati <- as.function(var)
  }
  if (Statistic = "mean") {
    stati <- as.function(mean)
  }
  else if (is.function(Statistic) != TRUE) {
  ...
  # Code if everything is okay
    ...
    thêta_star <- rep(0,B)
     ...
     # for the bootstrap estimate : list with the statistic evaluated on the sample x
  for (i in 1:B) {
  thêta_hat[i] <- p(sample(data, replace = TRUE))                    
  }
  
   # for the bootstrap estimate : on the bootstrapped samples (on têta_hat)
  for (i in 1:B) {
  thêta_star[i] <- p(sample(thêta_hat, replace = TRUE))                    
  }
  
  # for varBoot
  n <- as.numeric (length(thêta_hat))
  varBoot <- as.numeric (p(thêta_hat) *  ((n - 1) / n))
